...
// ---------------------------------------------------------------------------
// Updater (thin wrappers; plugin does the heavy lifting from the renderer)
// ---------------------------------------------------------------------------
...
pub async fn updater_check(app: AppHandle) -> Value {
    ...
    match updater.check().await {
        ...
        Ok(None) => ok(json!({ "update": null })),
        Err(e) => err(e.to_string()),
    }
}

#[tauri::command]
pub async fn updater_download_and_install(app: AppHandle) -> Value {
    use tauri_plugin_updater::UpdaterExt;
    ...
    let update = match updater.check().await {
        Ok(Some(u)) => u,
        Ok(None) => return err("No update available to install"),
        Err(e) => return err(e.to_string()),
    };

    let _ = app.emit("updater:event", json!({ "event": "checking" }));
    let _ = app.emit(
        "updater:event",
        json!({
            "event": "available",
    ...
    match update
        .download_and_install(
            |chunk_length, content_length| {
                ...
                let transferred = chunk_length as f64;
            ...
            || {
                let _ = app.emit("updater:event", json!({ "event": "downloaded" }));
            },
        )
        .await
    {
        ...
        Err(e) => {
            let _ = app.emit(
                "updater:event",
                json!({ "event": "error", "error": e.to_string() }),
            );
            err(e.to_string())
        }
    }
...
#[tauri::command]
pub fn updater_quit_and_install(app: AppHandle) -> Value {
    app.restart();
    #[allow(unreachable_code)]
    ok(json!({}))
}
