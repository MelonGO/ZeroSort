...
**AI-powered desktop note-taking application with rich editing, linked notes, embedded planning tools, flexible tagging, and encrypted cloud sync.**

ZeroSort is a privacy-first desktop application that combines a powerful rich-text editor with bidirectional note linking, AI-driven note organization, hierarchical folder management, color-coded tags, embedded calendar and kanban blocks, and end-to-end encrypted S3 cloud synchronization. Built with Tauri and React 19, it runs natively on macOS, Windows, and Linux with local-first SQLite storage.
